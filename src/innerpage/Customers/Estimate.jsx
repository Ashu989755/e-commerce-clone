...
import { CallIcon, EmailIcon, LocationIcon, UserIcon } from "../../assets/image";
...
const Estimate = () => {
  ...
  const handleRejectEstimates = async () => {
    try {
      const apiData = {
        estimateId: estimateId,
        status: 3,
        ...
        <section className="px-6 py-20">
          <div className="container mx-auto pt-5">
            <h3 className="font-bold text-xl flex items-center mt-6">
              ...
              {/* </span> */}
              <span className="text-lg font-normal">Estimate</span>
            </h3>
                ...
                <div className="flex flex-col gap-4">
                  <div className="bg-[#fff] rounded-xl shadow-lg w-full p-6">
                    <div className="flex items-center gap-4 mb-3">
                      <div className="w-24 h-24 rounded-md overflow-hidden bg-[#2883DE]">
                        <img
                          src={estimateList?.businessImage}
                          alt=""
                    ...
                    <div className="flex flex-col gap-2 mb-4">
                      <p className="text-sm">
                        ...
                        <span className="text-gray-600 font-medium">
                          {moment(estimateList?.estimateDate).format(
                            "MMMM-DD-YYYY"
                          )}
                          ,
                          {moment
                            .utc(estimateList?.estimateDate)
                            .tz(userTimeZone)
                            .format("h:mm A")}
                        </span>
                      </p>
                    ...
                    <div className="p-6 bg-[#F7F7F7] rounded-lg flex justify-between items-center">
                      ...
                      <p className="text-gray-900 uppercase text-sm font-semibold">
                        ${estimateList?.balanceDue?.toFixed(2)}
                      </p>
                    </div>
                  </div>
                  {(estimateList?.estimateStatus === 1 && role !== "business") ? (
                    <div className="flex justify-end gap-3 mt-4 mb-7">
                      <button
                        onClick={handleRejectEstimates}
                        type="button"
                        className="bg-[#F05151] border-black rounded-lg px-4 py-4 font-semibold flex gap-2 items-center text-white text-sm w-full justify-center"
                      >
                        Reject
                      </button>
                      <button
                        onClick={handleAcceptEstimates}
                        type="button"
                        className="bg-[#303030] rounded-lg px-4 py-3 font-semibold flex gap-2 items-center text-white text-sm w-full justify-center"
                      >
                        Accept
                      </button>
                    </div>
                  ...
                  <div className="mt-4 overflow-auto ">
                    <table class="my_table ">
                      ...
                      <tr>
                        ...
                        <td>${estimateList?.balanceDue?.toFixed(1)}</td>
                      </tr>
                    </table>
                  </div>
                </div>
