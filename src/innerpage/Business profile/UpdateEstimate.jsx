...
import Add_customer_Manually from "../../modals/Add_customer_Manually";
import {
  businessSaveEstimateItems,
} from "../../apis/business/Profile";

import { toast } from "react-toastify";
import Add_Edit_Modal from "../../modals/Add_Edit_Modal";
import { useSelector } from "react-redux";
import { businessUpdateEstimates } from "../../apis/business/Documents";
...
function UpdateEstimate() {
  ...
  const [value, setValuee] = useState(new Date());
  ...
  const [estimateCustomers, setEstimateCustomers] = useState([]);
  console.log(estimateCustomers, "=======================");
  ...
  const openEditModal = (customer, index) => {
    console.log("BUTTON CLIIIIIIIIIKEED");
    console.log(customer, "CUSTOMERrrrrrrrrrrrrrrrrr");
  ...
  console.log(state, "------------");
  const userIdd = state?.businessId;
  const priceType = state?.estimateType;

  console.log(addItems);

  useEffect(() => {
    if (state?.estimateCustomers) {
      setEstimateCustomers(state.estimateCustomers);
    }
  }, [state]);

  useEffect(() => {
    if (state) {
      setValue("totalHours", state?.totalHour || "");
      setValue("totalPrice", state.totalAmount || "");
      setValue("businessName", state.businessName || "");
      setValue("selectLabor", state.numberOfLabour || "");
      setValue("laborPrice", state.labourPrice || "");
    }
  }, [state, setValue]);
  ...
  const handleSendEstimates = async (data) => {
    console.log("CLICKED");
    const jobOpeningDate = selectedTime
      ? new Date(
        `${value.toISOString().split("T")[0]}T${selectedTime}:00`
      ).toISOString()
      : null;

    const jobClosingDate = selectedEndTime
      ? new Date(
        `${value.toISOString().split("T")[0]}T${selectedEndTime}:00`
      ).toISOString()
      : null;
    ...
    try {
      ...
      formData.append("EstimatePriceType", priceType);
      formData.append("JobOpeningDate", jobOpeningDate || "");
      ...
      const res = await businessUpdateEstimates(formData);
      setEstimateId(res?.data?.data);
      handleSaveEstimateItems(res?.data?.data);
  ...
  const handleSaveEstimateItems = async (id) => {
    ...
    try {
      const apiData = {
        estimateId: id,
        customerEmail: estimateCustomers[0]?.customerEmail,
        customerName: estimateCustomers[0]?.firstName,
        firstName: estimateCustomers[0]?.firstName,
        lastName: estimateCustomers[0]?.lastName,
        customerContactNumber: estimateCustomers[0]?.phoneValue,
        customerCountryCode: "",
        address1: estimateCustomers?.customerAddress1,
        address2: estimateCustomers[0]?.customerAddress2,
        zipcode: estimateCustomers[0]?.customerZipcode,
        city: estimateCustomers[0]?.customerCity,
        state: estimateCustomers[0]?.customerState,
        country: estimateCustomers[0]?.customerCountry,
  ...
  const handleRemoveCustomer = (index) => {
    setEstimateCustomers((prevItems) =>
          ...
          <div className="container mx-auto">
            <div className="grid grid-cols-2 gap-5">
              <div className="lg:col-span-1 col-span-2">
                ...
                <div className="border shadow-md rounded-xl p-5 bg-white mt-4">
                ...
                <div className="col-span-1 mt-4">
                  <div className="col-sm-1 relative">
                    <div className="my_pro border shadow-md rounded-xl p-5 bg-white">
                      ...
                      <input
                        type="time"
                        className="w-full outline-0 bg-[#f3f1f1] py-4 px-6 rounded-lg mt-3 shadow-sm focus:shadow-md border border-transparent focus:border-text_dark"
                        {...register("startTime")}
                        // value={selectedTime}
    ...
    shadow-sm focus:shadow-md border border-transparent focus:border-text_dark"
                      ...
                      {/* <div className="col-sm-1 relative">
                        ...
                        <svg width="24" height="24" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
                          <path d="M8.0008 22H16.0008C20.0208 22 20.7408 20.39 20.9508 18.43L21.7008 10.43C21.9708 7.99 21.2708 6 17.0008 6H7.0008C2.7308 6 2.0308 7.99 2.3008 10.43L3.0508 18.43C3.2608 20.39 3.9808 22 8.0008 22Z" stroke="#303030" stroke-width="1.5" stroke-miterlimit="10" stroke-linecap="round" stroke-linejoin="round"/>
                          <path d="M8 6V5.2C8 3.43 8 2 11.2 2H12.8C16 2 16 3.43 16 5.2V6" stroke="#303030" stroke-width="1.5" stroke-miterlimit="10" stroke-linecap="round" stroke-linejoin="round"/>
                          <path d="M14 13V14C14 14.01 14 14.01 14 14.02C14 15.11 13.99 16 12 16C10.02 16 10 15.12 10 14.03V13C10 12 10 12 11 12H13C14 12 14 12 14 13Z" stroke="#303030" stroke-width="1.5" stroke-miterlimit="10" stroke-linecap="round" stroke-linejoin="round"/>
                          <path d="M21.65 11C19.34 12.68 16.7 13.68 14 14.02" stroke="#303030" stroke-width="1.5" stroke-miterlimit="10" stroke-linecap="round" stroke-linejoin="round"/>
                          <path d="M2.61914 11.2695C4.86914 12.8095 7.40914 13.7395 9.99914 14.0295" stroke="#303030" stroke-width="1.5" stroke-miterlimit="10" stroke-linecap="round" stroke-linejoin="round"/>
                          </svg>
                        </span>
                        <select id="inputState" class="form-select w-full outline-0 bg-[#f3f1f1] py-4 px-10 rounded-lg 
                          shadow-sm focus:shadow-md border border-transparent focus:border-text_dark">
                      ...
                      {estimateCustomers?.length > 0 ? (
                        <div className="customer-list">
                          <h2 className="text-gray-800 pb-3 font-semibold">Customer Detail</h2>
                          {estimateCustomers?.map((customer, index) => (
                            <div
                              key={index}
                              className="flex items-start p-4 bg-gray-100 rounded-lg w-full justify-between"
                            >
                              <ul className="list-disc ps-4 space-y-2">
                                ...
                                <li className="text-sm text-gray-600">
                                  {customer.customerAddress1 || customer.address.customerAddress1},{" "}
                                  {customer.customerAddress2 || customer.address.customerAddress2},{" "}
                                  {customer.customerCity || customer.address.customerCity},{" "}
                                  {customer.customerState || customer.address.customerState}
                                </li>
                                <li className="text-sm text-gray-600">
                                ...
                                <li className="text-sm text-gray-600">
                                  {customer.countryCode
                                    ? `${customer.countryCode} ${customer.customerPhoneno}`
                                    : customer.phoneValue}
                                </li>
                              </ul>
                              <div className="flex flex-col gap-4 ml-4">
                                ...
                                >
                                  <span>
                                    <svg
                                      ...
                                      xmlns="http://www.w3.org/2000/svg"
                                    >
                                      <rect width="16" height="16" rx="8" fill="#F05151" />
                                      <g clipPath="url(#clip0_2119_13288)">
                                        <path
                                          d="M5.74722 10.6668L10.668 5.74609L10.2556 5.33369L5.33481 10.2544L5.74722 10.6668Z"
                                          fill="white"
                                ...
                                >
                                  ...
                                  >
                                    <path
                                      d="M2 21.5V17.5L14.12 5.38L18.12 9.38L6 21.5H2Z"
                                      fill="#000"
                                    />
                                    <path
                                      d="M20.71 7.04L16.71 3.04L18.83 0.91C19.22 0.52 19.82 0.52 20.21 0.91L23.09 3.79C23.48 4.18 23.48 4.78 23.09 5.17L20.71 7.04Z"
                                      fill="#000"
                                    />
                                  </svg>
                                </button>
                        ...
                        <button onClick={openModalEdit}>Add Customer Manually</button>
                      )}

                      <div className="inp gap-3">
                        <button
                          onClick={handleSubmit(handleSendEstimates)}
                          className="bg-dark_link w-full py-4 px-10 text-white font-medium rounded-lg"
                        >
                          Update Estimates
                        </button>
                      </div>
      ...
      <Add_Edit_Modal
        isOpen={isOpenManual}
        closeModalManual={closeModalManual}
        setEstimateCustomers={setEstimateCustomers}
        customerToEdit={customerToEdit}
        editIndex={editIndex}
        setResetCustomerForm={setResetCustomerForm}
